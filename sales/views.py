...
class SaleViewset(viewsets.ModelViewSet):
    ...
    def stats(self, request):
        """Statistiques des ventes (version corrigée avec TruncMonth)"""
        ...
        top_customers = Customer.objects.annotate(
            total_spent_calc=Sum(
                'sales__total', filter=Q(sales__status='delivered'))
        ).filter(total_spent_calc__isnull=False).order_by('-total_spent_calc')[:5]

        # Revenus mensuels avec TruncMonth (compatible SQLite)
            ...
            'top_customers': [
                {
                    'id': c.id,
                    'name': c.full_name,
                    'total_spent': float(c.total_spent_calc)
                }
        ...
        try:
            ...
            for quote_item in quotation.items.all():
                if not quote_item.warehouse:
                    ...
                    elif stock_quantity < quote_item.quantity * 1.1:
...
class DeliveryViewset(viewsets.ModelViewSet):
    ...
    def start(self, request, pk=None):
        ...
        if delivery.status != 'pending':
            return Response({"error": "Livraison déjà en cours"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def confirm(self, request, pk=None):
        ...
        if delivery.status != 'in_transit':
            return Response({"error": "Livraison non en transit"}, status=status.HTTP_400_BAD_REQUEST)
        delivery.status = 'delivered'
        delivery.save()
        sale = delivery.sale
        all_delivered = all(item.remaining_quantity ==
                            0 for item in sale.items.all())
        if all_delivered:
...
class ReturnViewset(viewsets.ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        if return_obj.status != 'pending':
            return Response({"error": "Retour déjà traité"}, status=status.HTTP_400_BAD_REQUEST)
