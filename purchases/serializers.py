...
class SupplierDetailSerializer(serializers.ModelSerializer):
    ...
    delivery_terms_display = serializers.CharField(
        source='get_delivery_terms_display', read_only=True)
...
# purchases/serializers.py - CORRECTION
...
class PurchaseOrderListSerializer(serializers.ModelSerializer):
    ...
    warehouse_id = serializers.IntegerField(  # ← AJOUTER
        source='warehouse.id', read_only=True, default=None)
    warehouse_name = serializers.CharField(  # ← AJOUTER
        source='warehouse.name', read_only=True, default=None)
    items = serializers.SerializerMethodField()

    class Meta:
        model = PurchaseOrder
        fields = ('id', 'order_number', 'supplier_name', 'order_date', 'expected_date',
                  'status', 'status_display', 'urgency', 'urgency_display', 'total',
                  'items_count', 'supplier_reference', 'warehouse_id', 'warehouse_name', 'items')
...
class PurchaseOrderCreateUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'expected_date',
            'urgency',
            'warehouse',  # ← AJOUTER CE CHAMP ICI
            'shipping_address',
            'notes',
            'internal_notes',
            'terms_conditions',
    ...
    def validate(self, data):
        # La validation de l'adresse de livraison n'est plus obligatoire si warehouse est présent
    ...
    def create(self, validated_data):
        ...
        # warehouse peut être None, c'est accepté
...
# ✅ CORRECTION ICI
# purchases/serializers.py - VERSION CORRIGÉE


# purchases/serializers.py - VERSION CORRIGÉE

# purchases/serializers.py - Ajout de la mise à jour du stock
...
class PurchaseReceiptCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_items(self, value):
        """Valide chaque article de la réception"""
        if not value:
    ...
    def create(self, validated_data):
        ...
        # Créer les lignes de réception ET mettre à jour le stock
        for item_data in items_data:
            order_item = item_data.pop('order_item_obj')
            quantity_received = item_data['quantity']

            # 1. Créer l'item de réception
            ...
            # 2. Mettre à jour la quantité reçue dans la commande
            order_item.quantity_received += quantity_received
            order_item.save()

            # 3. Mettre à jour le stock dans l'entrepôt
            # PASSER item_data À LA MÉTHODE
            self._update_stock_on_receipt(
                purchase_order=purchase_order,
                order_item=order_item,
                quantity=quantity_received,
                item_data=item_data  # ← AJOUTER CET ARGUMENT
            )
    ...
    def _update_stock_on_receipt(self, purchase_order, order_item, quantity, item_data=None):
        """
        Met à jour le stock dans l'entrepôt lors de la réception
        """
        from inventory.models import StockMovement, Warehouse, Lot, StockAlert
        from django.db import transaction

        try:
            with transaction.atomic():
                # Récupérer l'entrepôt
                if hasattr(purchase_order, 'warehouse') and purchase_order.warehouse:
                    ...
                    if not warehouse:
                        ...
                        if not warehouse:
                            raise Exception("Aucun entrepôt configuré")
